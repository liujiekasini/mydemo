...
public class UserDaoImpl extends DbUtilsBaseDao implements UserDao {
	...
	public User findUserByUsername(String username) {
		String sql = "select username as username,password as password from t_user where username=?";
		// User user = jdbcTemplate.queryForObject(sql, new Object[] { username
		// },
		// new BeanPropertyRowMapper<User>(User.class));
		String roles = "select tr.id,tr.rolename as name from t_role tr left join t_user_role  tur on tr.id=tur.role_id left join t_user tu on tur.user_id=tu.id where tu.username=? ";
	...
	public Paging searchmonitor(int page, int rows) throws SQLException {
		...
		String sql = "select login_name as loginName,ip as ip,date_format(create_time,'%y-%m-%d %h:%m:%s') as createTime from t_monitor";
