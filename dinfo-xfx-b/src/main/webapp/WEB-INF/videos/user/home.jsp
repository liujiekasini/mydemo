...
<!-- 		  					<td style="width:450px;"><input type="text" id="company_one" class="input-text size-M"></td> -->
		    ...
		    <div class="layui-tab-item">
		    	<div>
		    	  <table class="table0">
		  				<tr>
		  					<td style="text-align: right;"><span>关系词：</span></td>
		  					<td style="width:450px;"><textarea class="textarea" id="relateContent"></textarea></td>
		  				</tr>
		  				<tr>
		  					<td style="text-align: right;">关系词属性：</td>
		  					<td style="width:450px;">
		  						  nt<input type="radio" name="relateType" value="nt" title="包含停用词" checked="checked">
								  nr <input type="radio" name="relateType" value="nr" title="后缀停用词">
		  					</td>
...
function company_model_guanxi(){
	...
	var relateType=$('.table0 input[name="relateType"]:checked ').val();
	if(relateContent == "" ){
		layer.msg("不能为空！", {icon: 10, time: 1000});
		return false;
	}
...
function showProjectInfo(page){
	...
	function formatProjectInfoForTable(data){
		if(data.total!=0){
			var content="<tr style='border-bottom:1px solid #919191;'><td>公司简称</td><td>公司全称</td><td>创建时间</td></tr>";
...
function check_company_M(page){
	...
	function formatProjectInfoForTable_M(data){
		if(data.total!=0){
			var content="<tr style='border-bottom:1px solid #919191;'><td>人名</td><td>创建时间</td></tr>";
			for(var i=0;i<data.data.length;i++){
				var row=data.data[i];
				content+="<tr>";
...
function check_company_R(page){
	...
	function formatProjectInfoForTable_R(data){
		if(data.total!=0){
			var content="<tr style='border-bottom:1px solid #919191;'><td>关系词</td><td>词性</td><td>创建时间</td></tr>";
			for(var i=0;i<data.data.length;i++){
				var row=data.data[i];
				content+="<tr>";
				content+="<td>"+row['relation']+"</td>";
				content+="<td>"+row['relationNation']+"</td>";
				content+="<td>"+row['createTime']+"</td>";
				content+="</tr>";
			}
            $(".select_industry_table").attr("style","border-bottom:1px solid #919191;");
...
function check_company_T(page){
	...
	function formatProjectInfoForTable_T(data){
		if(data.total!=0){
			var content="<tr style='border-bottom:1px solid #919191;'><td>停用词</td><td>词性</td><td>创建时间</td></tr>";
			for(var i=0;i<data.data.length;i++){
				var row=data.data[i];
				content+="<tr>";
